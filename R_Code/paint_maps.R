...
plot_map <-function(mapa,data_to_plot,vxlim=c(),vylim=c(),psize=1.5,valpha=0.7,mg="Variation",model="",lposition="right")
{
  ...
  mapa_lens <- ggplot() +
    geom_polygon(data = mapa, aes(x=long, y = lat, group = group), 
                 fill="white",col="wheat4", alpha=0.1, lwd=0.2)
  if (mg == "BIO12") {
  ...
  mapa_lens<- mapa_lens+guides(shape = guide_legend(order = 2),col = guide_colourbar(order = 1),fill = guide_colourbar(order = 1))
    ...
    if (mg=="BIO1"){
      ...
      salto <- 10
      vector_breaks <- seq(-max(abs(data_to_plot$magnitude)),max(abs(data_to_plot$magnitude)),by=salto)
    }
  ...
  mapa_lens <- mapa_lens + theme(plot.title = element_text(vjust = -10, hjust = 1, 
                                                           size = 9, color="grey50")) + ggtitle(etq_title)  
...
AddMultiplotLabel <- function(pl,pvjust=-0.5)
{
  return (pl + plot_annotation(tag_levels = 'A') &
    theme(plot.tag = element_text(size = 11,face="bold",family=" Times New Roman",hjust = 0, vjust = pvjust)))
}
...
encircle_all_interest_samples <- FALSE
...
for (magnitudplot in vmags)
{
  ...
  plims <- c(-10,45)
  pl_map <- plot_map(mapa,data_to_plot,psize=1.1,vxlim=plims,mg=magnitudplot,model=CChange_model,
                        valpha=alpha_dots)
  ...
  vprecip_map_lens <- plot_map(mapa,data_to_plot,psize=1.8,vxlim=plims,mg="BIO12",
...
mapa_lens_present <- AddVHInterest(mapa_lens_present,data_to_plot)+xlab("          ")
...
mapa_lens_changes <- mapa_lens_present / (mapa_lens_variation +xlab("            "))/ (vprecip_map_lens[[1]] +ylab("Latitude")) 
mapa_lens_changes <- AddMultiplotLabel(mapa_lens_changes,pvjust=-10)
...
SavePlot(paste0(odir,"/mapa_lens_ALL_CHANGES_",CChange_model,"_",modelprec),mapa_lens_changes, pwidth=8, pheight=11,eps=printepsfile)
...
histsp <- AddMultiplotLabel(histsp)
...
high_value_samples <- data_to_plot[data_to_plot$VeryHighValue,]
